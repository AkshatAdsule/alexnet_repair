...
class EditSetVisualizer:
    ...
    def load_class_labels(self):
        ...
        # Create mappings for both class_id -> name and name -> class_id
        class_labels = {}  # class_id -> name
        name_to_id = {}  # name -> class_id

        for class_id, class_name in enumerate(cifar10_classes):
            class_labels[str(class_id)] = class_name
            name_to_id[class_name] = class_id

        print(f"Loaded {len(class_labels)} CIFAR-10 class labels")
        return class_labels, name_to_id
    ...
    def get_class_name(self, class_id):
        ...
        if isinstance(class_id, (int, float)):
            ...
            # If it's already a string, check if it's a name or ID
            if class_id in self.wordnet_to_name.values():
                return class_id  # It's already a class name
        else:
    ...
    def load_dataset_info(self):
        ...
        # For CIFAR-10, we don't need to load from file system
        # The class information is already in load_class_labels()
        self.class_to_idx = {
            name: idx
    ...
    def get_image_tensor_as_base64(self, image_tensor):
        ...
        # For CIFAR-10, the images are already normalized to [0,1] range typically
        # But let's handle both normalized and unnormalized cases
